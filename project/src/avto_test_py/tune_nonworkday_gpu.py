"""
tune_nonworkday_gpu.py  (project/src/avto_test_py/)
====================================================
Автоматический подбор гиперпараметров LightGBM — специалист ВЫХОДНЫХ ДНЕЙ (workday == 0).
Читает конфиг из project/configs/s_config.json.

Запуск (из корня репозитория):
    python project/src/avto_test_py/tune_nonworkday_gpu.py
"""
...
SPEC_CFG     = CFG['specialists']['nonworkday']
...
SPECIALIST = 'nonworkday'; WORKDAY_FLAG = 0
...
print(f'\nOptuna: {N_TRIALS} триалов (nonworkday, GPU)', flush=True)
done_flag=[False]
